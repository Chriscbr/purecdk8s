...
func NewWebCacheDB(scope constructs.Construct, id string) cdk8s.Chart {
	...
	memoryNodes := cdk8splus34.Node_Labeled(cdk8splus34.NodeLabelQuery_Is(jsii.String("optimized"), jsii.String("memory")))

	db := cdk8splus34.NewStatefulSet(chart, jsii.String("DB"), &cdk8splus34.StatefulSetProps{
		Containers: &[]*cdk8splus34.ContainerProps{{Image: jsii.String("db"), PortNumber: jsii.Number(8000)}},
	...
	cache := cdk8splus34.NewDeployment(chart, jsii.String("Cache"), &cdk8splus34.DeploymentProps{
		Containers: &[]*cdk8splus34.ContainerProps{{
			Image:      jsii.String("cache"),
			PortNumber: jsii.Number(7000),
			EnvVariables: &map[string]cdk8splus34.EnvValue{
				"DB_HOST": cdk8splus34.EnvValue_FromValue(db.Service().Name()),
				"DB_PORT": cdk8splus34.EnvValue_FromValue(jsii.String(strconv.FormatFloat(*db.Service().Port(), 'f', 0, 64))),
			},
		}},
		Replicas: jsii.Number(2),
		Spread:   jsii.Bool(true),
	...
	web := cdk8splus34.NewDeployment(chart, jsii.String("Web"), &cdk8splus34.DeploymentProps{
		Containers: &[]*cdk8splus34.ContainerProps{{
			Image:      jsii.String("web"),
			PortNumber: jsii.Number(6000),
			EnvVariables: &map[string]cdk8splus34.EnvValue{
				"CACHE_HOST": cdk8splus34.EnvValue_FromValue(cacheService.Name()),
				"CACHE_PORT": cdk8splus34.EnvValue_FromValue(jsii.String(strconv.FormatFloat(*cacheService.Port(), 'f', 0, 64))),
			},
		}},
		Replicas: jsii.Number(2),
		Spread:   jsii.Bool(true),
	...
	frontoffice := cdk8splus34.Group_FromName(chart, jsii.String("FOGroup"), jsii.String("frontoffice"))
