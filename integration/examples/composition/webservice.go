...
func NewWebService(scope constructs.Construct, id *string, props *WebServiceProps) constructs.Construct {
	service := constructs.NewConstruct(scope, id)
	...
	labels := map[string]*string{"app": cdk8s.Names_ToLabelValue(service, nil)}

	k8s.NewKubeService(service, jsii.String("service"), &k8s.KubeServiceProps{
	...
	k8s.NewKubeDeployment(service, jsii.String("deployment"), &k8s.KubeDeploymentProps{
