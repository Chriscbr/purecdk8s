...
func NewElasticsearchQuery(scope constructs.Construct, id string) cdk8s.Chart {
	...
	passwordSecret := cdk8splus34.Secret_FromSecretName(chart, jsii.String("Secret"), jsii.String(fmt.Sprintf("%s-es-elastic-user", *elastic.Name())))

	deployment := cdk8splus34.NewDeployment(chart, jsii.String("Deployment"), &cdk8splus34.DeploymentProps{
	...
	container := deployment.AddContainer(&cdk8splus34.ContainerProps{
		...
		EnvVariables: &map[string]cdk8splus34.EnvValue{
			"ELASTIC_USERNAME": cdk8splus34.EnvValue_FromValue(jsii.String("elastic")),
			"ELASTIC_ENDPOINT": cdk8splus34.EnvValue_FromValue(jsii.String(fmt.Sprintf("http://%s-es-http:%d", *elastic.Name(), es_port))),
			"ELASTIC_PASSWORD": cdk8splus34.EnvValue_FromSecretValue(&cdk8splus34.SecretValue{Secret: passwordSecret, Key: jsii.String("elastic")}, nil),
		},
	})

	configMap := cdk8splus34.NewConfigMap(chart, jsii.String("Config"), nil)
	configMap.AddFile(jsii.String("./query.js"), nil)

	volume := cdk8splus34.Volume_FromConfigMap(chart, jsii.String("Volume"), configMap, nil)
